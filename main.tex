...
\begin{table}\centering
  ...
  \midrule
    ...
    $M$ & \texttt{0.0} & \texttt{4.499e-24}\\
    $s$ & \texttt{0.0} & \texttt{1.419e-05}\\
  \bottomrule
\end{tabularx}
...
\begin{figure}
\input{perf.tex}
  \caption{Performance comparison of the conversion from CIECAM02 to XYZ (with
  $J$, $C$, and $h$). Making use of the suggested improvments in the inverse
  model, colorio is more than 30\% faster.}
\end{figure}
...
improved CAM16 algorithm are given here.
The steps that differ from the original model are marked with an asterisk (*).
As noted above, all improvements carry over to CIECAM02 as well.
...
parameters}
(See the note at the end of Part 2 of Appendix B for determining
all parameters)
...
end of Part 1 of Appendix A.
\item $N_c$ and $F$ are modelled as a function of $c$, and their values
can be linearly interpolated (see Figure A1), using the above
points
\end{itemize}
